<template>
	<div id="video-src">
		<div class="src src-video f-cb" v-if="mvData">
			<div class="video f-pr j-flag" :class="{'f-hide':!videHide}" @click.stop="videHideClick(true)">
				<div class="info f-pa" @click.stop="videHideClick(true)">
					<div class="tit u-dicn u-dicn-17">
						<p class="f-thide2">
							<span class="h3">{{mvData.name}}</span>
							<span class="h4">-by{{mvData.artistName}}</span>
						</p>
					</div>
				...
				<img :src="mvData.imgurl+'?param=338y189'" class="f-img" @click.stop="videHideClick(true)"/>
			</div>
			<div class="fold f-cb j-flag " :class="{'f-hide':videHide}">
						...
						<div class="inf f-thide s-fc3">
							<router-link :to="{path:'/mv',query:{id:mvData.Id}}" class="btn s-fc3">
								<i class="icn u-dicn u-dicn-35"></i>
								{{mvData.name}}
							</router-link>
							<router-link :to="{path:'/user/home',query:{id:AItem.id}}" v-for="(AItem,AIndex) in mvData.artists" class="sub s-fc4">
								{{AIndex==mvData.artists.length-1&&AItem.anme||(AItem.name + '/')}}
							</router-link>
						</div>
					</div>
						...
						<div class="m-ctvideo z-active z-pause"  :class="[video.playing?'z-play':'z-pause',{'z-fullscreen':definition},{'z-end':video.zEnd}]">
							<div class="player">
								...
								<div class="poster ffull">
									<img :src="mvData.imgurl" class="j-pic pic"/>
								</div>
...
//          debugger;
		console.log(this.mvData)
		 this.mvHttp(this.mvData.id)
		 this.videoHeight = (this.videoWidth/this.mvData.width)*this.mvData.height
		 this.mvData.playCount = this.$Playback(this.mvData.playCount)
		 this.mvData.duration = this.$formatTime(this.mvData.duration/1000)
        },
...
//      	全屏播放
        	...
        	mvHttp(id){
        		...
        		_this.getHttp('/mv/url?id='+id,function(res){
