<template>
	<div id="user-record" v-if="msg">
		...
		<div class="m-record" :data-songs="recordData.length">
			<div v-if="recordData">
...
<script>import eventVue from '../../../../static/js/eventVue.js'
export default {
	name: 'songsRank',
	data() {
		return {
			...
			this.$http({
				...
				if(resp.data.allData != undefined) {
					if(resp.data.allData.length > 0) {
						...
						that.$.each(resp.data.allData, function(index, el) {
							el.idData = id;
							let arTitle = '';
					...
					if(resp.data.weekData.length > 0) {
						...
						that.$.each(resp.data.weekData, function(index, el) {
							el.idData = id;
							let arTitle = '';
							that.$.each(el.ar, function(num, item) {
								console.log(item)
