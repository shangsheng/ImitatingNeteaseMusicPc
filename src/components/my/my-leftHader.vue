			...
			<div v-for="(item,index) in subcount" v-if="item.countfalse&&subCountData[item.name]!=0">
					...
					<div class="n-minelst n-minelst-1" v-else-if="subCountData[item.name]!=0" :class="{'n-minelst-hide':!item.playlistFalse}">
						<h2 class="f-ff1">
							...
							<span class="rtitle f-pr cursor" data-action="spread"  @click.stop="createPlaylist(item.playlistFalse,index)">
								<i class="tri tri1"></i>
								{{item.title}}
	    ...
	    this.$root.eventVue.$emit('barVoice',this.barVoice);
		  ...
		  console.log(this.$route)
	         if(this.$cookieUtil("email")){
					...
					if(that.$route.name == 'musicArtist'){
						that.indexZn = null;
						that.jsSelected = 0;
					}else if(that.$route.name == 'musicMv'){
						that.indexZn = null;
						that.jsSelected = 2;
					}else if(that.$route.name == 'musicRadio'){
						that.indexZn = null;
						that.jsSelected = 1;
					}
