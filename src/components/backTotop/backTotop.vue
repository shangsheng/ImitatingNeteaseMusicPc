<template>
  <transition :name="transitionName">
    <div class="page-component-up"
      ...
      v-show="visible"
      :style="customStyle">
      <i class="el-icon-caret-top m-back">返回顶部</i>
    </div>
  </transition>
</template>
    ...
    backToTop() {
      ...
      this.interval = setInterval(() => {
        ...
        if(next < this.backPosition) {
          window.scrollTo(0, 0)//this.backPosition
          clearInterval(this.interval)
        } else{
...
.page-component-up{
  ...
  i{
	...
	.m-back, .m-lycifo .downapp, .m-plshare .btn, .m-pubcode .code, .m-multi .bg, .m-multi .bg a:hover {
	    background: url(../../assets/sprite.png) no-repeat 0 9999px;
	}
</style>