						...
						<div class="n-mv">
							<div class="title f-cb">
								<h2 class="f-ff2 f-thide" title="">
									<i class="f-fl tag u-icn2 u-icn2-mvtag"></i>
									{{mvData.name}}
								</h2>
								...
								<div class="m-ctvideo z-active " tabindex="1" :class="[video.playing?'z-play':'z-pause',{'z-fullscreen':definition}]">
									<div class="player">
										...
										<div class="poster ffull" >
											<img :src="mvData.cover" class="j-pic pic"/>
										</div>
										<!--屏幕上的播放键-->
										...
										<div class="opbar">
											...
											<h2 class="j-tt tt fthide" >{{mvData.name}}-{{mvData.artistName}}</h2>
										</div>
							...
							<div class="btns f-cb">
								...
								<span class="j-flag btn u-btni u-btni-fav cursor" data-action="sub" :class="{'u-btni-unfav':faDis}" :data-liked="mvData.id" :data-res-id="mvData.id" :data-count="mvData.subCount" @click.stop="likeClick($event)">
									<i>
										({{mvData.subCount}})
									</i>
								</span>
			...
			<div class="g-sd4" v-if="mvData">
				<div class="g-wrap7">
					...
					<div class="m-mvintr">
						...
						<p class="s-fc4">播放次数:{{mvData.playCount}}次</p>
					...
					<ul class="n-mvlist f-cb" v-if="xgmv">
						<li v-for="(item,index) in xgmv">
							<div class="u-cover u-cover-8 f-fl">
								...
								<router-link :to="{path:'/mv',query:{id:item.id}}" class="f-link link" :title="item.name"></router-link>
							</div>
							<div class="cnt">
								<p class="tit f-thide">
									<i class="tag u-icn2 u-icn2-smvtag"></i>
									<router-link :to="{path:'/mv',query:{id:item.id}}" :title="item.name">{{item.name}}</router-link>
								</p>
...
//          debugger;
		...
		if(this.$cookieUtil("email")){
					this.$logoRefresh(function(ress){
						 that.$http({
				         	method:'get',
				         	url:that.$host+"/mv/sublist?uid="+ress.data.profile.userId
				         }).then(function(res){
				         	console.log(res.data)
				         	that.$.each(res.data.data,function(index,el){
				         		console.log(el.vid)
				         		
				         		console.log(el.vid == id)
						       if(el.vid == id){
						       	that.faDis=true;
...
//          debugger;
			let _this = this;
        	let id = this.$route.query.id;
			this.getHttp('/simi/mv?mvid='+id,function(res){
				console.log(res)
				_this.$.each(res.mvs,function(index,el){
					el.duration = _this.$formatTime(el.duration/1000)
					el.playCount = _this.$Playback(el.playCount)
				})
				_this.xgmv = res.mvs
			})
        },
...
//      	全屏播放
        	...
        	mvHttp(){
        		let _this = this;
        		let id = this.$route.query.id;
        		_this.getHttp('/mv/detail?mvid='+id,function(res){
				     console.log(res.data)
				     let artistsName = ''
				     _this.$.each(res.data.artists,function(index,el){
				     	if(index<res.data.artists.length){
				     		artistsName = el.name  + '/'
				     	}else {
				     		artistsName = el.name + artistsName
				     	}
				     })
...
//		         	console.log(res.data)
        	...
        	videoHttp(id){
        		let _this = this;
        			_this.getHttp('/mv/url='+id,function(res){
					     console.log(res)
